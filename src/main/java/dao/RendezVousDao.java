...
public class RendezVousDao {
	...
	public static int insertRv(RendezVous rdv) {
		...
		try {
			...
			PreparedStatement ps = con.prepareStatement("insert into rendezvous (date_rv,patient_idpatient,service_nom_service) values (?,?,?)" );
	...
	public static void updateRv(RendezVous rdv) {
		...
		try {
			...
			Connection con = RendezVousDao.getconnection();
			PreparedStatement ps = con.prepareStatement(" update RendezVous set  date_rv=?,patient_idpatient=? where idRendezVous = ?" );
			ps.setDate(1, (Date) rdv.getDate_rv());
			ps.setInt(2, rdv.getPatient_idpatient());
			ps.setInt(3, rdv.getIdRendezvous());
			ps.executeUpdate();
	...
	public static RendezVous getRvById(int id) {
		...
		try {
			...
			Connection con = RendezVousDao.getconnection();
			PreparedStatement ps = con.prepareStatement(" select idRendezvous,date_rv,patient_idpatient,service_nom_service from rendezvous where idRendezVous = ?" );
			ps.setInt(1, id);
	...
	public static List<RendezVous> getAllRendezVousById(int id){
	...
	try {
			...
			PreparedStatement ps = con.prepareStatement(" SELECT * FROM rendezvous where patient_idpatient = ? ");
			ps.setInt(1, id);
			ResultSet rs = ps.executeQuery();
	...
	public static List<RendezVous> getAllRendezVousByNomService(String id){
	...
	try {
			...
			PreparedStatement ps = con.prepareStatement(" SELECT * FROM rendezvous where service_nom_service = ? ");
			ps.setString(1, id);
			ResultSet rs = ps.executeQuery();
